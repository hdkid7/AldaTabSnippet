...
import RedirectMaker from "./RedirectMaker";
import {
...
const viewReplace = string => string.replace("/view-", "");
...
const RouteViewBuiler = () => {
  const listItems = object.map((obj, index) => (
    <Route key={index} path={viewReplace(`/${obj.tabName.toLowerCase()}`)}>
      <TabContent>
...
const NavBuilder = () => {
  const listItems = object.map((obj, index) => (
    <NavLinkMaker key={index} route={viewReplace(obj.tabName.toLowerCase())}>
      {obj.tabName}
    </NavLinkMaker>
  ));
...
const RedirectBuilder = () => {
  const listItems = object.map((obj, index) => (
    <RedirectMaker key={index} tabName={obj.tabName.toLowerCase()} />
  ));
  return <Fragment>{listItems}</Fragment>;
};
  ...
  return (
    <Router>
      ...
      <Redirect
        exact
        from="/"
        to={viewReplace(`/${object[0].tabName.toLowerCase()}`)}
      />
    </Router>
  );
};
